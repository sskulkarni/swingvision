...
class CourtReference:
    ...
    def __init__(self):
        ...
        # --- Always use the absolute path for the reference image ---
        court_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'court_configurations'))
        os.makedirs(court_dir, exist_ok=True)
        self.court_img_path = os.path.join(court_dir, 'court_reference.png')

        # If the image does not exist, build and save it
        if not os.path.exists(self.court_img_path):
            print(f"Court reference image not found at {self.court_img_path}, generating now...")
            self.build_court_reference()

        self.court = cv2.cvtColor(cv2.imread(self.court_img_path), cv2.COLOR_BGR2GRAY)
    ...
    def build_court_reference(self):
        ...
        cv2.imwrite(self.court_img_path, court)
        self.court = court
        return court
    ...
    def save_all_court_configurations(self):
        ...
        for i, conf in self.court_conf.items():
            ...
            cv2.imwrite(os.path.join(os.path.dirname(self.court_img_path), f'court_conf_{i}.png'), c)
